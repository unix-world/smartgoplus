...
// based on github.com/quickjs-go/quickjs-go, license MIT
// (c) 2020-2021 FlyingtimeICE <flyingtimeice@gmail.com>
// (c) 2020 Kenta Iwasaki <kenta@lithdew.net>
// (c) 2017-2020 Fabrice Bellard
// (c) 2017-2020 Charlie Gordon
...
func (ctx *Context) Function(fn Function) Value {
	val := ctx.eval(`(proxy, id) => function() { return proxy.call(this, id, ...arguments); }`, EVAL_GLOBAL)
	if val.IsException() {
		return val
	}
	defer val.Free()
...
func (ctx *Context) eval(code string, evaltype int) Value {
	var filename string = "<code>"
...
func (ctx *Context) EvalBinary(code []byte, evaltype int) (Value, error) { // by unixman ; sync with js_std_eval_binary() from quickjs-libc.c
	codePtr := (*C.uint8_t)(C.CBytes(code))
	defer C.free(unsafe.Pointer(codePtr))
	obj := C.JS_ReadObject(ctx.ref, codePtr, C.size_t(len(code)), C.JS_READ_OBJ_BYTECODE)
	if(C.JS_IsException(obj) == 1) {
		return Value{ctx: ctx, ref: C.JS_NewNull()}, ctx.Exception()
	}
	if (evaltype == EVAL_MODULE) {
		if(C.JS_ResolveModule(ctx.ref, obj) < 0) {
			C.JS_FreeValue(ctx.ref, obj);
			return Value{ctx: ctx, ref: C.JS_NewNull()}, ctx.Exception()
		}
		C.js_module_set_import_meta(ctx.ref, obj, C.int(0), C.int(1));
	}
	val := Value{ctx: ctx, ref: C.JS_EvalFunction(ctx.ref, obj)}
	if val.IsException() {
		return val, ctx.Exception()
	}
	return val, nil
