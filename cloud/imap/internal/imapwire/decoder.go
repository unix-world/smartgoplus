...
import (
	...
	"github.com/unix-world/smartgoplus/cloud/imap/internal/imapnum"
	"github.com/unix-world/smartgoplus/cloud/imap/internal/utf7"
)
...
func (dec *Decoder) SP() bool {
	if dec.acceptByte(' ') {
		// github.com / emersion / go-imap / issues / 571
		b, ok := dec.readByte()
		if !ok {
			return false
		}
...
func (dec *Decoder) CRLF() bool {
	dec.acceptByte(' ')  // github.com / emersion / go-imap / issues / 540
...
func (dec *Decoder) ExpectBodyFldOctets(ptr *uint32) bool {
	...
	// size. See:
	// github.com / emersion / go-imap / issues / 534
	if dec.acceptByte('-') {
		*ptr = 0
