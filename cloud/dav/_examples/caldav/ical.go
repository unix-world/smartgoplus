...
func TestFilter(t *testing.T) {
	newCO := func(str string) caldav.CalendarObject {
...
END:VCALENDAR`)
	...
	for _, tc := range []struct {
		...
		addrs []caldav.CalendarObject
		want  []caldav.CalendarObject
		err   error
	}{
		{
			name:  "nil-query",
			query: nil,
			addrs: []caldav.CalendarObject{event1, event2, event3, todo1},
			want:  []caldav.CalendarObject{event1, event2, event3, todo1},
		},
		{
			...
			addrs: []caldav.CalendarObject{event1, event2, event3, todo1},
			want:  []caldav.CalendarObject{event1, event2, event3},
		},
		{
			...
			addrs: []caldav.CalendarObject{event1, event2, event3, todo1},
			want:  []caldav.CalendarObject{event2, event3},
		},
		{
			// https://datatracker.ietf.org/doc/html/rfc4791#section-7.8.1
		...
		},
		{
			...
			addrs: []caldav.CalendarObject{event1, event2, event3, todo1},
			want:  []caldav.CalendarObject{event3},
		},
		{
			// https://datatracker.ietf.org/doc/html/rfc4791#section-7.8.6
			...
			addrs: []caldav.CalendarObject{event1, event2, event3, todo1},
			want:  []caldav.CalendarObject{event1},
		},
		{
			...
			addrs: []caldav.CalendarObject{event1, event2, event3, todo1},
			want:  []caldav.CalendarObject{event2},
		},
		// TODO add more examples
	} {
		t.Run(tc.name, func(t *testing.T) {
			got, err := caldav.Filter(tc.query, tc.addrs)
			switch {
