...
import (
	...
	"github.com/unix-world/smartgoext/data-structs/tidwall/gjson"
	"github.com/unix-world/smartgoplus/data-structs/tidwall/tinylru"
)
...
//-- unixman io-util ReadDir() is deprecated, this is a copy of it
...
func (l *Log) load() error {
	fis, err := readDir(l.path) // unixman: use an internal method instead of deprecated io-util
	if err != nil {
		return err
	}
	startIdx := -1
...
func (l *Log) loadSegmentEntries(s *segment) error {
	data, err := os.ReadFile(s.path) // unixman use os instead of io-util
	if err != nil {
		return err
	}
	ebuf := data
